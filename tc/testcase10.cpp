...
int main()
{
    ...
    */
    ...
    graph.addConnection(t1, a1, 6867);
    graph.addConnection(d, e, 3247);
    graph.addConnection(e1, f, 4567);
    graph.addConnection(k, l, 12387);
    ...
    graph.addConnection(b1, c1, 1);
    graph.addConnection(b1, t, 123123);
    graph.addConnection(c1, q1, 1);
    graph.addConnection(t, e1, 32943);
    graph.addConnection(e1, s1, 239230);
    graph.addConnection(q1, s1, 1);
    ...
    std::cout << "Total Import of: " << k.getCountry() << " " << graph.getTotalImports(k) << std::endl;

    //Cyclic
    std::cout << "isCyclic ?  --->  " << graph.isCyclic() << std::endl;

    //FindLeast
    std::list<string> path;
    path = graph.findLeastCostPath(b1, s1);
